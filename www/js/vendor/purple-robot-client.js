...
// PurpleRobotClient.serverUrl = "http://10.101.118.36:12345/json/submit";
...
PurpleRobotClient.emitToast = function (string, duration, emission_format) {
    js_to_execute = 'PurpleRobot.emitToast(\"' + string +'\",'+ duration+');'
    return PurpleRobotClient.actionHandler(js_to_execute, emission_format);
}

PurpleRobotClient.showNativeDialog = function (title, message, confirm_title, cancel_title, confirm_script, cancel_script, emission_format) {
    js_to_execute = 'PurpleRobot.showNativeDialog(\"'+title+'\",\"'+message+'\",\"'+confirm_title+'\",\"'+confirm_script+'\",\"'+cancel_script+'\");'
    return PurpleRobotClient.actionHandler(js_to_execute, emission_format);
}
